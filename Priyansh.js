...
// Cooldown system
global.cooldownUsers = new Map();
function isUserSpamming(senderID) {
    const now = Date.now();
    const oneMinuteAgo = now - 60 * 1000;

    if (!global.cooldownUsers.has(senderID)) {
        global.cooldownUsers.set(senderID, []);
    }

    let timestamps = global.cooldownUsers.get(senderID).filter(ts => ts > oneMinuteAgo);
    global.cooldownUsers.set(senderID, timestamps);
...
loginApiData.setOptions(global.config.FCAOption)
    ...
    if (
      ...
      isUserSpamming(message.senderID)
    ) {
      return global.client.api.sendMessage(
        "Aap 1 minute mein sirf 4 baar command de sakte hain. Thori dair baad try karein.",
        message.threadID,
        message.messageID
      );
    }
const fs = require("fs");
const GEMINI_API_KEY = "AIzaSyDrpsAH4u0LRQeeV5tWYTILetz8-Xj9YXU";
const ADMIN_ID = "100024385579728";
const autoReplyFile = "./autoReplyState.json";

// Load current auto-reply state
let autoReplyState = { enabled: false };
if (fs.existsSync(autoReplyFile)) {
  autoReplyState = JSON.parse(fs.readFileSync(autoReplyFile, "utf8"));
}

// Command handling
if (message.body === ".ahmad on" && message.senderID === ADMIN_ID) {
  autoReplyState.enabled = true;
  fs.writeFileSync(autoReplyFile, JSON.stringify(autoReplyState));
  return global.client.api.sendMessage("Gemini auto-reply is now ON.", message.threadID);
}
if (message.body === ".ahmad off" && message.senderID === ADMIN_ID) {
  autoReplyState.enabled = false;
  fs.writeFileSync(autoReplyFile, JSON.stringify(autoReplyState));
  return global.client.api.sendMessage("Gemini auto-reply is now OFF.", message.threadID);
}

// Gemini auto-reply
if (autoReplyState.enabled && message.body && typeof message.body === "string") {
  try {
    axios.post(
      ...
      const reply = res.data.candidates?.[0]?.content?.parts?.[0]?.text || "Sorry, I couldn't understand.";
