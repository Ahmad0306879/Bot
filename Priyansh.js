...
if (
  ...
  try {
    const res = await axios.get(`https://api.llama-api.com/chat?q=${encodeURIComponent(message.body)}`);
    const reply = res.data.answer || "Mujhe jawab nahi mila.";
    return global.client.api.sendMessage(reply, message.threadID, message.messageID);
  } catch (err) {
